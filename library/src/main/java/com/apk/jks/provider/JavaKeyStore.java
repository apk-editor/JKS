...
import com.apk.jks.utils.IOUtils;
        ...
        synchronized (entries) {
            ...
            for (Enumeration<String> e = entries.keys(); e.hasMoreElements(); ) {
                ...
                if (entry instanceof KeyEntry) {
                    ...
                    // Write the (entry creation) date
                    dos.writeLong(((TrustedCertEntry) Objects.requireNonNull(entry)).date.getTime());
                ...
                if (tag == 1) { // private key entry
                    ...
                    if (numOfCerts > 0) {
                        ...
                        for (int j = 0; j < numOfCerts; j++) {
                            ...
                            certs.add(Objects.requireNonNull(cf).generateCertificate(bais));
                            bais.close();
                        }
                    ...
                    encoded = IOUtils.readFully(dis, dis.readInt(), true);
                    bais = new ByteArrayInputStream(encoded);
                    entry.cert = Objects.requireNonNull(cf).generateCertificate(bais);
                    bais.close();
