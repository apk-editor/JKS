...
final public class ObjectIdentifier implements Serializable {
     ...
     */
    private static int pack7Oid(byte[] in, int ilength, byte[] out, int ooffset) {
        byte[] pack = pack(in, 0, ilength, 8, 7);
    ...
    private static int pack7Oid(int input, byte[] out, int ooffset) {
        ...
        b[2] = (byte)(input >> 8);
        b[3] = (byte)(input);
        return pack7Oid(b, 4, out, ooffset);
    }
    ...
    private static int pack7Oid(BigInteger input, byte[] out, int ooffset) {
        ...
        return pack7Oid(b, b.length, out, ooffset);
    }
    ...
    private static void checkSecondComponent(int first, BigInteger second) throws IOException {
        if (second.signum() == -1 ||
                first != 2 &&
                        second.compareTo(BigInteger.valueOf(39)) > 0) {
            throw new IOException("ObjectIdentifier() -- " +
                    "Second oid component is invalid ");
        }
    }
